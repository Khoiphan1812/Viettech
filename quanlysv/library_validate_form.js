...
RegisterForm.prototype.validateField = function (fieldName, text) {
  ...
  if (field.isTheoryScoreValid) {
    if (!this.isTheoryScoreValid(text)) {
      throw new Error(field.isTheoryScoreValid);
    }
  }
  if (field.isBirthYear) {
    if (!this.isBirthYear(text)) {
      throw new Error(field.isBirthYear);
    }
  }
};
